...
def registrar_producto():
    ...
    if request.method == "POST":
        ...
        try:
            #Verificar que los cmpos esten llenos
            if float(precio_u) < 0:
                flash("No se aceptan numeros negativos en el precio", "danger")
            ...
            if not str(nombre):
                flash("Llena el campo de nombre", "danger")
                return render_template("admin/productos/registrar_producto.html", categorias=categorias, nombre=nombre, precio_u=precio_u, fk_cat=fk_cat, contenido=contenido, stock=stock, marca=marca, cod_barras=cod_barras)
...
def editar_producto(producto_id):
    ...
    if request.method == 'POST':
        ...
        if not precio_u:
            flash("El campo de precio no puede estar vacío.", "danger")
            return render_template('admin/productos/editar_producto.html', producto=producto, categorias=categorias)

        try:
            if not str(nombre):
                flash("El campo del nombre no puede estar vacío.", "danger")
                return render_template('admin/productos/editar_producto.html', producto=producto, categorias=categorias)

            precio_u = to_float(precio_u)
            stock = to_float(stock)

            if precio_u is None or stock is None:
                return render_template('admin/productos/editar_producto.html', producto=producto, categorias=categorias)

            if precio_u < 0:
                flash("El precio de los productos NO puede ser negativo.", "danger")
                return render_template('admin/productos/editar_producto.html', producto=producto, categorias=categorias)

            if stock < 0:
                flash("La cantidad de producto no puede ser negativa.", "danger")
                return render_template('admin/productos/editar_producto.html', producto=producto, categorias=categorias)
...
def registrar_venta():
    ...
    if request.method == 'POST':
        try:
            ...
            if float(cant_prod) < 1:
                flash("No se aceptan cantidades menores a 1", "danger")
                return render_template('admin/ventas/registrar_venta.html', categorias=categorias, mesas=mesas, productos=productos)
